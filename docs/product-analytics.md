...
The last command requires Playwright and its Chromium browser in the Node environment. Its API is a local synthetic fixture; it does not use the running Keys app or send reports externally. Swift tests use temporary catalogs, a fake transport and in-memory credentials. Collector tests use a temporary database and loopback HTTP sockets. Live deployment and Apple signing are separate validation steps.
