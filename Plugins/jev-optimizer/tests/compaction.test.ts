...
describe('decisions', () => {
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[keys-jev-optimizer truncated ${total - 50} chars of this tool result; consult the original transcript or re-read only if safe]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[keys-jev-optimizer truncated ${total} chars of this tool result; consult the original transcript or re-read only if safe]`,
    );
  });
});
...
describe('compact', () => {
  it('resends the full state with every batch and merges the answers', async () => {
    ...
    const output = await compact(
      ...
      { minRemovableChars: 0, preserveRecentMessages: 1, maxRequestTokens: stateTokens + 150 },
    );
    ...
    expect(output.stats).toMatchObject({ resultsDropped: 2, kept: 0, callsDropped: 0, pinned: 0, errorsKept: 1 });
  ...
  it('reports a tiny reduction when Jev wants everything kept', async () => {
    const output = await compact(transcript(), fakeJev(() => 0.95), { minRemovableChars: 0, preserveRecentMessages: 1 });
  ...
  it('rejects malformed answers', async () => {
    ...
    await expect(compact(transcript(), broken, { minRemovableChars: 0, preserveRecentMessages: 1 })).rejects.toThrow(
      /Invalid evaluation answer/,
    );
  });
});

describe('HTTP client', () => {
  it('builds a gateway evaluation request', () => {
    ...
    // `noul` goes out as the gateway's `boolean`; the model travels in a header, not the body.
  ...
  it('rejects failed and malformed responses', () => {
    ...
    expect(
      parseJevResponse(
        ...
        JSON.stringify({
          answers: {
            b: { type: 'boolean', probability: 0.7 },
      ...
      answers: {
        ...
        s: { type: 'score', score: 1.5, confidence: 1, probabilities: {} },
      },
  ...
  it('asks over fetch and refuses to run without a key', async () => {
    ...
    const client = new JevClient({
      ...
      fetch: (async (_url: string | URL | Request, init?: RequestInit) => {
        models.push(new Headers(init?.headers).get('ai-model-id') ?? '');
    ...
    expect(models).toEqual(['jev-test']);

    const keyless = new JevClient({ apiKey: '' });
    await expect(keyless.ask('s', {})).rejects.toThrow(/AI_GATEWAY_API_KEY/);
    await expect(
      compactMessages(transcript(), { apiKey: '', minRemovableChars: 0, preserveRecentMessages: 1 }),
