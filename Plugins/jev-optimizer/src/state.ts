...
export const STATE_CONTEXT =
  'A coding assistant conversation is being compacted to free context. `history` is the whole conversation so far, oldest first; tool outputs are replaced by a short `result` note and long texts may be abridged. Each question asks whether one tool call, or the full output of that call, still needs to stay in the history verbatim. Keep uncertain evidence. Removed output may be unavailable later, and tools can have side effects; never assume they are safe to re-run.';
...
/**
 * Heuristic request-token estimate: a word costs one token per six letters,
 * a digit half a token, any other symbol nine tenths. This is not a provider
 * tokenizer or a guarantee of the final request size/cost.
 */
...
/** Known instruction files and coordination/tool-loading calls stay verbatim. */
export function isProtectedCall(tool: string, input: Record<string, unknown>): boolean {
  const normalized = tool.replace(/([a-z0-9])([A-Z])/g, '$1_$2').toLowerCase();
  if (/(?:^|[._-])(?:skill|tool_search|load_tools|spawn_agent|send_message|request_user_input|ask_user_question|task_output|task_create|task_update|task_list|task_get|task_stop|list_agents|task|agent)(?:$|[._-])/.test(normalized)) return true;
  let text: string;
  try { text = JSON.stringify(input); } catch { return true; }
  return /(?:^|[\/\\\s"'])?(?:AGENTS|CLAUDE|SKILL)\.md(?:$|[\s"'\\])/i.test(text);
}
...
export function collectToolCalls(
  ...
  messages.forEach((message, index) => {
    for (const tool of message.toolUses) useCounts.set(tool.tool_use_id, (useCounts.get(tool.tool_use_id) ?? 0) + 1);
    for (const result of message.toolResults ?? []) {
      resultCounts.set(result.tool_use_id, (resultCounts.get(result.tool_use_id) ?? 0) + 1);
  ...
  messages.forEach((message, callIndex) => {
    for (const tool of message.toolUses) {
      ...
      // Ambiguous/reversed pairs stay untouched; never let one decision delete several blocks.
      if (!found || found.index <= callIndex || useCounts.get(tool.tool_use_id) !== 1 || resultCounts.get(tool.tool_use_id) !== 1) continue;
      calls.push({
        id: `t${calls.length + 1}`,
        ...
        isError: Boolean(found.result.isError || tool.isError),
        protected: isProtectedCall(tool.tool, tool.input),
        pinned:
