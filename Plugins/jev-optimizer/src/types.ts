...
export interface ToolCall {
  ...
  pinned: boolean;
  /** Known instruction-loading or coordination output; never removed. */
  protected?: boolean;
}
...
export interface CallDecision extends CallAnswer {
  ...
  tool: string;
  action: CallAction;
  reason: 'pinned' | 'error' | 'protected' | 'kept' | 'result_dropped' | 'call_dropped';
}
...
export interface CompactOptions {
  ...
  goal?: string;
  /** Minimum keep probability for a call or result to stay. Default 0.2 (uncertainty keeps). */
  ...
  /** Skip the evaluator unless eligible result text could remove this many characters. Default 4000. */
  minRemovableChars?: number;
  /** Score only the largest eligible outputs; others stay untouched. Default 16. */
  maxCandidates?: number;
  /** Total estimated input across all batches, including repeated state. Default 60000. */
  maxDecisionInputTokens?: number;
  /** Maximum evaluation batches for one compaction. Default 3. */
  maxRequests?: number;
  /** Maximum concurrent evaluation requests. Default 2. */
...
export interface CompactResult {
  ...
  stats: {
    ...
    skippedReason?: 'no_candidates' | 'below_minimum';
    stateTokens: number;
    /** Which fitting stage the state needed, '' when no request was made. */
    stateStage: string;
    /** Actual transport requests made by this compaction, excluding decision reuse. */
    requests: number;
    cacheHits: number;
    /** Estimated evaluator input for uncached requests; not realized savings. */
    estimatedDecisionInputTokens: number;
    /** Actual reported evaluator usage; null if any uncached batch omitted/invalidated that count. */
    jevUsage: { inputTokens: number | null; outputTokens: number | null; complete: boolean };
    ms: number;
  };
}
